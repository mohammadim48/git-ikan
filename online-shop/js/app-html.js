				...
				var SelectModal = {
					...
					setData: function setData(data) {
						...
						$.each(data.variants, function (key, value) {
							...
							$(that.defaults.options, $popup).append($('<option ' + option_status + '>', {
								value: value.id
							}).text(key + value.info));
					...
					anime({
						...
						complete: function complete() {
							...
							if (!$('.minicart-prd').length) {
								$(that.defaults.empty).removeClass('d-none').css({
									'opacity': 0,
									'height': 0
								});
			...
			if ($(el).length) {
				THEME.mobileFilter = {
					...
					setShift: function setShift(delay) {
						var that = this,
							left = $(that.options.mobileFilter).hasClass('filter-col--opened') ? $(that.options.mobileFilter).offset().left : $(that.options.grid).offset();
				...
				_handlers: function _handlers() {
					$horGrid.on('click', function (e) {
						...
						if (!$this.is('.active')) {
							...
							$('.list-options [data-toggle="tooltip"]', $products).attr('data-placement', 'left').tooltip('dispose');
							$('.prd-circle-labels [data-toggle="tooltip"]', $products).attr('data-placement', 'right').tooltip('dispose');
					...
					$list.on('click', function (e) {
						...
						if (!$this.is('.active')) {
							...
							$('[data-toggle="tooltip"]', $products).attr('data-placement', 'right').tooltip('dispose');
							THEME.initialization.tooltipIni($('[data-toggle="tooltip"]', $products));
							THEME.product.productWidth('.prd, .prd-hor, .prd-promo');
							setTimeout(function () {
								$products.removeClass('off-animation');
			...
			$contactForm.validate({
				...
				messages: {
					username: {
						...
						minlength: "نام کاربری شما می بایست حداقل شامل 2 کاراکتر باشد"
					},
